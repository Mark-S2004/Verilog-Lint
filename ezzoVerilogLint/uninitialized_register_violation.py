...
def check_uninitialized_registers(verilog_code):
    ...
    for each_reg in declared_registers:
        initialized_regs.append({"name":each_reg['name'] , "loc": re.finditer(r'\b' + each_reg["name"] + r'\s*<?=', verilog_code)})

    appeared_regs = re.finditer(r"=\s*?(?P<left_operand>[a-zA-Z]\w*)(\s*?[+*-]\s*?(?P<right_operand>[a-zA-Z]\w*))?" , verilog_code)
            ...
            if appeared_reg.group("left_operand") == initialized_reg["name"]:
                ...
                for loc in initialized_reg["loc"]:
                    ...
                    if loc.start() < appeared_reg.start():
                        ...
                        break
                if not init:
                    uninitialized_registers.append((verilog_code.count("\n", 0, appeared_reg.start()) , appeared_reg.group("left_operand")))
            if appeared_reg.group("right_operand" ) == initialized_reg["name"]:
                ...
                for loc in initialized_reg["loc"]:
                    ...
                    if loc.start() < appeared_reg.start():
                        ...
                        break
                if not init:
                    uninitialized_registers.append((verilog_code.count("\n", 0, appeared_reg.start()) , appeared_reg.group("right_operand")))
        if not found:
            uninitialized_registers.append((verilog_code.count("\n", 0,  appeared_reg.start()) , appeared_reg.group("left_operand")))
